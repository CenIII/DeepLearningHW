...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        l2norm = torch.sum((output1-output2)**2,1)
        eucdist = l2norm**(0.5)
        label = label.squeeze()
        loss_contrastive = torch.mean((1-label) * l2norm + label * (torch.clamp(self.margin - eucdist, min=0.0)**2))
...
if __name__ == "__main__":
    ...
    plt.plot(list(range(Config.train_number_epochs)), np.array(loss_history))
