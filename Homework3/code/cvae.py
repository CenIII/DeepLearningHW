...
class CVAE(nn.Module):
    def __init__(self, input_size, latent_size, class_size):
        ...
        self.generate = nn.Sequential(
                        ...
                        nn.Linear(self.units,self.units),
                        nn.ReLU(),
                        nn.Linear(self.units,self.input_size)
                        )
        ######################################################
        ###               END OF YOUR CODE                 ###
        ######################################################
    ...
    def recognition_model(self, x, c):
        ...
        ######### TO DO ###########
        ###########################
        xc_cat = torch.cat((x,c),1)
        z = self.recognize(xc_cat)
        mu = z[:,:self.latent_size]
        logvar = z[:,self.latent_size:]
        return mu, logvar
    ...
    def generation_model(self, z, c): # P(x|z, c)
        ...
        ######### TO DO ###########
        ###########################
        zc_cat = torch.cat((z,c),1)
        x_hat = self.generate(zc_cat)
        return x_hat
    ...
    def forward(self, x, c):
        ...
        sampled_z = self.reparametrize(mu,logvar)
        x_hat = self.generation_model(sampled_z,c)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # lat_dim = len(mu[0])
    # regularization loss
    reg_loss = -1/2*torch.sum((1+logvar-mu**2-torch.exp(logvar)),1)

    # reconstruction loss 
    recons_loss = torch.sum(((x_hat-x)**2),1)

    loss = torch.sum((reg_loss + recons_loss))/batch_size
    return loss
